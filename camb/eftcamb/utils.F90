    ...
    !if ( associated(R%points)  ) deallocate(R%points,stat = status) ! MR: this seems to be causing a bug
    !if ( associated(R%dpoints) ) deallocate(R%dpoints,stat = status) ! MR: this seems to be causing a bug
    ...
    if ( associated(R%points)  ) nullify(R%points)
    if ( associated(R%dpoints) ) nullify(R%dpoints)
    R%count = 0
    R%npoints = 0
    R%has_dpoints = .false.
