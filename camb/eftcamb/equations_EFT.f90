...
contains
    ...
    subroutine output(EV,y, j,tau,sources)
        ...
        ! EFT sources:
        real(dl) EFTetakdot, EFTsigmadotdot, EFT_PsiDot, EFT_PhiDot
        ! Debug quantities
        ...
        if (CP%EFTflag/=0 .and. a>=EFTturnonpi) then
            EFTetakdot =  ( 0.5_dl*dgq/(1._dl+EFTOmegaV) + k2/3._dl*EFTeomF/EFT_H0 + k2/3._dl*(EFTeomU-EFTeomX)*z )/EFTeomX

            EFTsigmadot= 1._dl/EFTeomX*(-2._dl*adotoa*(1._dl+EFTeomV)*sigma +etak&
                & -1._dl/k*dgpi/(1._dl+EFTOmegaV) +EFTeomN/EFT_H0)

            EFTsigmadotdot = 1._dl/EFTeomX*(-2._dl*(1._dl+EFTeomV)*(Hdot*sigma+adotoa*EFTsigmadot)&
                & -2._dl*adotoa*sigma*EFTeomVdot +EFTetakdot &
            ...
            EFT_PsiDot = +EFTsigmadotdot/k +adotoa*EFTsigmadot/k +Hdot*sigma/k
            EFT_PhiDot = +EFTetakdot/k -adotoa*EFTsigmadot/k -Hdot*sigma/k
        end if
        ! EFTCAMB MOD END
            ...
            if (tau>tau_start_redshiftwindows .or. plot_evolve) then
                ...
                do w_ix = 1, num_redshiftwindows
                    ...
                    elseif (W%kind == window_counts) then
                        ...
                        ! EFTCAMB MOD START: write phidot without assuming vanishing anisotropic stress. ! MR: OK
                        if (CP%EFTflag==0 .or. a<EFTturnonpi) then
                            !phidot neglecting anisotropic stress so phi=psi
                        ...
                        if (counts_redshift) then
                            ...
                            if (CP%EFTflag==0 .or. a<EFTturnonpi) then
                                ...
                                counts_redshift_source = +W%ddwing2(j)/k*sigma +W%dwing2(j)/k*(2._dl*k*EFT_Psi-2._dl*adotoa*sigma)&
                                    & +W%wing2(j)/k*(sigma*(adotoa**2-Hdot)-adotoa*k*EFT_Psi +k*EFT_PsiDot)
                            end if
                        ! EFTCAMB MOD END.
                        else
