...
contains
    ...
    subroutine DarkEnergy_ReadParams(P, Ini)
        use IniFile
        Type(TIniFile)   :: Ini
        Type(CAMBparams) :: P
        ...
        if(.not. use_tabulated_w)then
            P%w_lam = Ini_Read_Double_File(Ini,'w', -1.d0)
            P%wa_ppf = Ini_Read_Double_File(Ini,'wa', 0.d0)
            if (Feedback > 3) write(*,'("(w0, wa) = (", f8.5,", ", f8.5, ")")') P%w_lam, P%wa_ppf
        else
            write(*,*) 'CosmicFish does not work with tabulated w'
            stop
...
100         close(10)
    ...
    function w_de(a)
        ...
        if(.not. use_tabulated_w) then
            w_de= CP%w_lam+ CP%wa_ppf*(1._dl-a)
        else
            al=dlog(a)
    ...
    function grho_de(a)  !8 pi G a^4 rho_de
        ...
        if(.not. use_tabulated_w) then
            grho_de=grhov*a**(1._dl-3.*CP%w_lam-3.*CP%wa_ppf)*exp(-3.*CP%wa_ppf*(1._dl-a))
        else
            if(a.eq.0.d0)then
...
1       if (khi-klo.gt.1) then
    ...
    subroutine setcgammappf( P )

        implicit none

        Type(CAMBparams) :: P

        P%c_Gamma_ppf=0.4d0*sqrt(P%cs2_lam)
...
subroutine init_background
    ...
    is_cosmological_constant = .not. use_tabulated_w .and. CP%w_lam==-1_dl .and. CP%wa_ppf==0._dl
...
module GaugeInterface
    ...
    character(LEN=*), parameter :: Eqns_name = 'gauge_inv-ppf'
...
contains
    ...
    subroutine derivsv(EV,n,tau,yv,yvprime)
        ...
        grhor_t=grhornomass/a2
        grhog_t=grhog/a2
        grhov_t=grhov*a**(-1-3*CP%w_lam)

        grho=grhob_t+grhoc_t+grhor_t+grhog_t+grhov_t
        gpres=(grhog_t+grhor_t)/3._dl+grhov_t*CP%w_lam
