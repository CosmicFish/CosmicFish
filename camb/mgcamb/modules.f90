...
module RedshiftSpaceData
    ...
    procedure(generic_window), save, pointer :: window_function_calc => null()

    abstract interface
        function generic_window(Win, z, winamp)
            use precision
            import TRedWin
            implicit none
            real(dl) :: generic_window
            real(dl), intent(in) :: z
            real(dl) :: winamp
            Type(TRedWin) :: Win
        end function
...
contains
    ...
    ! COSMICFISH MOD START: we need to define the standard window
    function default_window(Win, z, winamp)

        Type(TRedWin) :: Win
        real(dl), intent(in) :: z
        real(dl) default_window, dz,winamp
        real(dl), parameter :: root2pi = 2.506628274_dl

        dz = z-Win%Redshift
        winamp =  exp(-(dz/Win%sigma)**2/2)
        default_window =winamp/Win%sigma/root2pi

    end function default_window
    ! COSMICFISH MOD END.
    ...
    function count_obs_window_z(Win, z, winamp)
        ...
        ! COSMICFISH MOD START: we need to associate the pointer, if it is not already associated, and then use it.
        if ( .not.associated(window_function_calc) ) then
            window_function_calc => default_window
        end if
        count_obs_window_z = window_function_calc(Win, z, winamp)
    ! original code:
    !dz = z-Win%Redshift
    !winamp =  exp(-(dz/Win%sigma)**2/2)
...
module ModelParams
    ...
    type CAMBparams
        ...
        real(dl) window_kmax_boost
        ! COSMICFISH MOD END.

        ! MGCAMB MOD START: adding modified gravity parameters to standard CAMB parameters:
        integer  :: MGC_model
        real(dl) :: GRtrans
        ! model 1:
        real(dl) :: B1, B2, lambda1_2, lambda2_2, ss
        ! model 2:
        real(dl) :: MGQfix, MGRfix
        ! model 3:
        real(dl) :: Qnot, Rnot, sss
        ! model 4 and 5:
        real(dl) :: B0
        ! model 6:
        real(dl) :: Linder_gamma
        ! model 7:
        real(dl) :: beta_star, xi_star, a_star
        ! model 8:
        real(dl) :: beta0, xi0, DilR, DilS
        ! model 9:
        real(dl) :: F_R0, FRn
        ! model 10:
        real(dl) :: A_2
...
contains
    ...
    subroutine inithermo(taumin,taumax)
        ...
        ! COSMICFISH MOD START: fixes a bug when calling this for tensors
        ...
        if ( .not. CP%WantTensors ) then
            do RW_i = 1, num_redshiftwindows
                ...
                if (Redshift_w(RW_i)%kind == window_lensing .or. &
                    ...
                    if (FeedbackLevel>0) &
                        & write(*,'(I3," Int W              = ",f9.6)') RW_i, awin_lens1(RW_i)
