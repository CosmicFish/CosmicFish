...
contains
    ...
    !-----------------------------------------------
    ! Feedback function:
    subroutine MGCAMB_feedback( P )

        implicit none

        type(CAMBparams), intent(in) :: P

        if ( P%MGC_model == 0 ) return

        write(*,*) "MGCAMB model: ", P%MGC_model
        write(*,*) "Transition to GR: ", P%GRtrans

        if ( P%MGC_model == 1 ) then
            write(*,*) 'B1 = ', P%B1
            write(*,*) 'B2 = ', P%B1
            write(*,*) 'lambda1_2 = ', P%lambda1_2
            write(*,*) 'lambda2_2 = ', P%lambda2_2
            write(*,*) 'ss = ', P%ss
        else if ( P%MGC_model == 2 ) then
            write(*,*) 'MGQfix = ', P%MGQfix
            write(*,*) 'MGRfix = ', P%MGRfix
            ...
            write(*,*) 'Linder_gamma = ', P%Linder_gamma
        else if ( P%MGC_model == 7 ) then
            write(*,*) 'beta_star = ', P%beta_star
            write(*,*) 'xi_star = ', P%xi_star
            write(*,*) 'a_star = ', P%a_star
    ...
    function MGMu(a,adotoa,k2,model)
        ...
        if(model==1 .or.model==4 .or.model==5.or.model==6) then
            ...
            LKA1 = CP%lambda1_2 * k2 * a**CP%ss
            LKA2 = CP%lambda2_2 * k2 * a**CP%ss

            MGMu = (1.d0 + CP%B1 * LKA1)/(1.d0 + LKA1)

            if (model ==4) then ! correction for f(R) mu function.
                MGMu = MGMu/(1.d0 - 1.4d-8 * CP%lambda1_2 * a**3)
            end if
            ...
            if ( model ==6) then
                ...
                MGMu=2.d0/3.d0*omm**(CP%Linder_gamma-1.d0)*&
                    &(omm**CP%Linder_gamma+2-3.d0*CP%Linder_gamma+3.d0*(CP%Linder_gamma-0.5d0)*omm)
    ...
    function MGMuDot(a,adotoa,k2,Hdot,model)
        ...
        integer , intent(in) :: model
        real(dl), intent(in) :: a, adotoa, k2, Hdot
        real(dl) :: MGMuDot
        real(dl) :: LKA1 ! \lambda_1^2 k^2 a^s
        real(dl) :: LKA2 ! \lambda_1^2 k^2 a^s
        real(dl) :: t1,t2,t1dot,t2dot
        real(dl) :: omm, ommdot
        real(dl) :: omde, omdedot, Hdotmy
        ...
        if(model==1 .or.model==4 .or.model==5.or.model==6) then
            ...
            MGMuDot = ((CP%B1 - 1.d0) * adotoa * CP%ss * LKA1) / ((1.d0+LKA1)**2.d0)

            if ( model ==4) then ! correction for f(R) mu function.
                MGMuDot = MGMuDot/(1.d0 - 1.4d-8 * CP%lambda1_2 * a**3) + 3.d0 * &
                    &MGMu(a,adotoa,k2,4)* adotoa *a**3 *(1.4d-8 *&
                    &CP%lambda1_2 )/(1.d0 - 1.4d-8 * CP%lambda1_2 * a**3)
            end if
            ...
            if ( model ==6) then
                ...
                MGMuDot = MGMu(a,adotoa, k2,6)/omm*(CP%Linder_gamma-1.d0)*ommdot+&
                    &2.d0/3.d0*omm**(CP%Linder_gamma-1.d0)*ommdot*&
                    &(CP%Linder_gamma*omm**(CP%Linder_gamma-1.d0)+3.d0*(CP%Linder_gamma-0.5d0))
    ...
    function MGGamma(a,adotoa,k2, model)
        ...
        if(model==1 .or.model==4 .or.model==5.or.model==6) then
            ...
            MGGamma = (1.d0 + CP%B2 * LKA2)/(1.d0 +LKA2)
    ...
    function MGGammaDot(a,adotoa,k2,Hdot,model)
        ...
        if(model==1 .or.model==4 .or.model==5.or.model==6) then
            ...
            MGGammaDot = ((CP%B2 -1.d0)*adotoa * CP%ss* LKA2)/((1.d0+LKA2)**2.d0)
    ...
    function MGM(a,adotoa,model)
        ...
        ! SYMMETRON
        if(model == 7) then
            MGM = (CP%H0/3.0D05) / (CP%xi_star) * sqrt(1.d0-(CP%a_star/a)**3.d0)

        ! DILATON: based on 1206.3568
        else if (model==8) then
            MGM = (CP%H0/3.0D05) /(CP%xi0) * a**(- CP%DilR)

        ! Hu-Sawicki f(R) model: m, beta parametrization as in 1305.5647
        else if (model == 9)then
            FRm0 = (CP%h0/3.0D05)*sqrt((4.d0*CP%omegav + CP%omegab + CP%omegac)/((CP%FRn+1.d0)*CP%F_R0))!note factor of c here
            MGM = FRm0 * ((4.d0 * CP%omegav + (CP%omegab + CP%omegac)*a**(-3.d0))/(4.d0 * CP%omegav + CP%omegab &
                + CP%omegac))**(CP%FRn/2.d0+1.d0)

        ! Simpler DILATON model
        else if (model ==10)then
            MGM = sqrt(3.d0*CP%A_2)*(adotoa/a)  ! H(a) = da/dtau/a**2 = adotoa/a
    ...
    function MGMDot(a, adotoa, Hdot,  model)
        ...
        ! SYMMETRON
        if(model == 7) then
            MGMDot = 1.5d0* (CP%H0/3.0D05)/(CP%xi_star) *((CP%a_star/a)**3.d0 * adotoa)/( sqrt(1.d0-(CP%a_star/a)**3.d0))

        ! DILATON
        else if (model==8) then
            MGMDot = - CP%DilR * MGM(a,adotoa,model) * adotoa ! complete this

        ! Hu-Sawicki f(R) model
        else if (model == 9)then
            FRm0 = (CP%h0/3.0D05)*sqrt((4.d0*CP%omegav + CP%omegab + CP%omegac)/((CP%FRn+1.d0)*CP%F_R0))
            MGMDot = MGM(a,adotoa,9) / (4.d0 * CP%omegav + (CP%omegab + CP%omegac)*a**(-3.d0)) * (-3.d0* CP%FRn / 2.d0 - 3.d0) *&
                ((CP%omegab + CP%omegac)* a**(-3.d0) * adotoa )!/(4.d0 * CP%omegav + CP%omegab + CP%omegac)) ! complete this

        ! Simple DILATON model
        else if (model ==10)then
            MGMDot = sqrt(3.d0*CP%A_2)*(Hdot- adotoa**2.d0)/a !/3.0D05
    ...
    function MGBeta(a, adotoa, model)
        ...
        ! SYMMETRON
        if(model == 7) then
            MGBeta =  CP%beta_star * sqrt(1.d0-(CP%a_star/a)**3.d0)

        ! DILATON
        else if (model==8) then
            MGBeta = CP%beta0 * exp((CP%DilS)/(2.d0* CP%DilR - 3.d0)*(a**(2.d0* CP%DilR - 3.d0)-1.d0))
    ...
    function MGBetaDot(a, adotoa, model)
        ...
        ! SYMMETRON
        if(model == 7) then
            MGBetaDot = 1.5d0 * (CP%beta_star * (CP%a_star/a)**3.d0 * adotoa) /( sqrt(1.d0-(CP%a_star/a)**3.d0))

        ! DILATON
        else if (model==8) then
            MGBetaDot = MGBeta(a, adotoa, 8) * (CP%DilS * a**(2.d0* CP%DilR - 3.d0) * adotoa)
    ...
    function MG_Q(a,adotoa, model)
        ...
        if (model ==2) then
            ...
            MG_Q = 1.d0 + (CP%Qnot - 1.d0)* a**CP%sss
    ...
    function MG_QDot(a,adotoa, model)
        ...
        if (model ==2) then
            ...
            MG_QDot = (CP%Qnot - 1.d0)*adotoa* CP%sss* a**(CP%sss)
        end if
    ...
    function MG_R(a,adotoa, model)
        ...
        if (model ==2) then
            ...
            MG_R = 1.d0 + (CP%Rnot - 1.d0)* a**CP%sss
    ...
    function MG_RDot(a, adotoa, model)
        ...
        if (model ==2) then
            ...
            MG_RDot = (CP%Rnot - 1.d0)*adotoa* CP%sss* a**(CP%sss)
...
contains
    ...
    subroutine output(EV,y, j,tau,sources)
        ...
        real(dl) :: MGC_Psi, MGC_Phi, MGC_PsiDot, MGC_Phidot
        !* MGCAMB mod: end
        !***********************************************************
        ...
        if ( CP%MGC_model == 7) then
            if (a< CP%a_star) then
                tempmodel = 0
            else
                tempmodel = CP%MGC_model
            end if
        else
            if ( a.lt. CP%GRtrans ) then
                tempmodel = 0
            else
                tempmodel = CP%MGC_model
            end if
        ...
        if (tempmodel /= 0) then
            ...
            if ( CP%MGC_model==1 .or.CP%MGC_model==4 .or.CP%MGC_model==5.or. &
                ...
                MG_gamma = MGGamma(a,adotoa,k2,CP%MGC_model)
                MG_gammadot = MGGammaDot(a,adotoa,k2,Hdot,CP%MGC_model)
            ...
            ! Q,R parametrization
            else if ( CP%MGC_model ==2.or.CP%MGC_model ==3) then

                MGQ = MG_Q(a,adotoa, CP%MGC_model)
                MGR = MG_R(a,adotoa, CP%MGC_model)
                MGQdot = MG_QDot(a,adotoa, CP%MGC_model)
                MGRdot = MG_RDot(a,adotoa, CP%MGC_model)
        ...
        !********************************************************
        if ( .not. CP%flat .and. CP%MGC_model /= 0 ) then
        ...
        if( tempmodel /= 0 ) then
            ...
            if( CP%MGC_model==1 .or. CP%MGC_model==4 .or. CP%MGC_model==5 .or. &
                &CP%MGC_model==6 .or. CP%MGC_model==7 .or. CP%MGC_model==8 .or. &
                &CP%MGC_model==9 .or. CP%MGC_model==10 .or. CP%MGC_model==11 .or. CP%MGC_model==12) then
                MG_psidot = ( -MG_mudot*( MG_rhoDelta + 2.d0* dgpi) -MG_mu*( MG_rhoDeltadot + 2.d0* dgpidot) )/(2.d0*k2)
        ...
        if (CTransScal%NumSources > 2) then
            ...
            ! MGCAMB MOD START:prepare to build the sources
            MGC_Psi    = MG_psi
            MGC_Phi    = MG_phi
            MGC_PsiDot = MG_psidot
            MGC_Phidot = MG_phidot
            ! MGCAMB MOD END.
            ...
            if (tau>taurend .and. CP%tau0-tau > 0.1_dl) then
                ...
                if( tempmodel == 0 ) then
                    ...
                    sources(3) = -(MGC_Psi+MGC_Phi)*f_K(tau-tau_maxvis)/(f_K(CP%tau0-tau_maxvis)*f_K(CP%tau0-tau))
                end if
                ! MGCAMB mod end
                !*******************************************************
            ...
            if (tau>tau_start_redshiftwindows .or. plot_evolve) then
                ...
                do w_ix = 1, num_redshiftwindows
                    ...
                    if (W%kind == window_lensing) then
                        ! MGCAMB MOD START: lensing source:
                        if(tempmodel == 0 ) then
                    ...
                    elseif (W%kind == window_counts) then
                        ...
                        ! MGCAMB MOD START: phidot computation
                        if(tempmodel == 0 ) then
                        ...
                        if (counts_redshift) then
                            ! MGCAMB MOD START: redshift source
                            if( tempmodel == 0 ) then
                                ...
                                counts_redshift_source = +W%ddwing2(j)/k*sigma +W%dwing2(j)/k*(2._dl*k*MGC_Psi-2._dl*adotoa*sigma)&
                                    & +W%wing2(j)/k*(sigma*(adotoa**2-Hdot)-adotoa*k*MGC_Psi +k*MGC_PsiDot)
                            end if
                            ! MGCAMB MOD END.
                        else
                        ...
                        if (CP%tau0-tau > 0.1_dl .and. counts_radial) then
                            chi =  CP%tau0-tau
                            ! MR: no need to modify. This term is cancelled once we have all the contributions. Might still be worth checking.
                        ...
                        if (counts_timedelay) then
                            ...
                            if ( tempmodel == 0 ) then
                                ...
                                counts_timedelay_source= 2*(1-2.5*W%dlog10Ndm)*W%WinV(j)*(MGC_Phi+MGC_Psi)
                            end if
                            ! MGCAMB MOD END.
                        else
                        ...
                        if (counts_ISW) then
                            ...
                            if ( tempmodel == 0 ) then
                                ...
                                counts_ISW_source = W%WinF(j)*(MGC_PhiDot+MGC_PsiDot)
                            end if
                            ! MGCAMB MOD END.
                        else
                            counts_ISW_source = 0
                        ...
                        if (counts_potential) then
                            ...
                            if ( tempmodel == 0 ) then
                                ...
                                counts_potential_source = ( MGC_PhiDot/adotoa + MGC_Psi +(5*W%dlog10Ndm-2)*MGC_Phi ) * W%wing(j) + MGC_Psi*W%wingtau(j)
                            end if
                            ! MGCAMB MOD END.
                        else
                        ...
                        if (counts_velocity) then
                            ...
                            if ( tempmodel == 0 ) then
                                ...
                                counts_velocity_source =  W%dwingtau(j)/k*sigma +W%wingtau(j)/k*(-adotoa*sigma +k*MGC_Psi) &
                                    - counts_radial_source  !don't double count terms; counts_radial is part of counts_velocity with 1/H/chi
                            end if
                            ! MGCAMB MOD END.
                        else
                        ...
                        if (DoRedshiftLensing) then
                            if ( tempmodel == 0 ) then
                                ...
                                sources(3+W%mag_index+num_redshiftwindows) = 0.5_dl*(MGC_Phi+MGC_Psi)*W%win_lens(j)*(2-5*W%dlog10Ndm)
                            end if
                        end if
                        ! MGCAMB MOD END.
    ...
    subroutine derivs(EV,n,tau,ay,ayprime)
        ...
        ! MGCAMB MOD START: works only with flat models
        if (CP%flat) then
            adotoa=sqrt(grho/3)
        ...
        ! MGCAMB MOD START: anisotropic contribution from massive neutrinos
        dgpi = 0
        if (CP%Num_Nu_Massive > 0) then
