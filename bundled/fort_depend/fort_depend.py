...
#Definitions
def run(files=None,verbose=True,overwrite=None,output=None,macros={}, path=""):
    ...
    tmp=write_depend(outfile=output,dep=depends,overwrite=overwrite, path=path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p','--path',nargs=1,help='Path to append to all dependencies')
    ...
    run(files=args.files, verbose=args.verbose, overwrite=args.overwrite, macros=macros, output=output, path=path)
