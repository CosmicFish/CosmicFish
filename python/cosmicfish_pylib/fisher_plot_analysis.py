    ...
    def search_fisher_path(self, fisher_path, search_fisher_guess=False, with_derived=True):
        ...
        for file in fisher_files:
            ...
            try:
                ...
                if __feedback__ > 1: print('SUCCESS')
                if __feedback__ == 1: print('Imported as a Fisher matrix: '+file)
                # remove from the derived fishers if necessary:
                derived_fisher_files.remove(file)
            except:
                if __feedback__ > 1: print('FAIL')
        # import derived fisher matrices:
        if with_derived:
            for file in derived_fisher_files:
                # get the derived matrix:
                if __feedback__ > 1: print('Trying to import: '+file+' as a derived Fisher matrix: ', end=' ')
                try:
                ...
                if fisher_derived is not None:
                    for i in range(len(self.fisher_name_list)):
                        if __feedback__ > 1: print('Trying to add derived from: '+fisher_derived.name+' to the Fisher matrix: '+self.fisher_list[i].name, end=' ')
                        try:
                            ...
                            if __feedback__ > 1: print('SUCCESS')
                            if __feedback__ == 1: print('Added derived parameters from: '+fisher_derived.name+' to Fisher matrix: '+self.fisher_list[i].name)
                        except:
                            if __feedback__ > 1: print('FAIL')
    ...
    def compute_gaussian(self, params=None, confidence_level=0.68, names=None, num_points=100, normalized=False, nice_bounds=True):
        ...
        for par1 in params_temp:
            ...
            for mat,name in zip(fisher_temp_list,names_temp):
                ...
                x_points = np.linspace(lower_bound, upper_bound, int(num_points))
                # get y:
                if normalized:
