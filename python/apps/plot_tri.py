...
Simple Python code to do triangular plot of a set of Fisher matrices.

The ouput will be a pdf file with the plot.

Invoking the help option ``plot_triangle.py -h`` will result in::

    usage: plot_triangle.py [-h] [-o OUTROOT] [-p PARAMS [PARAMS ...]] [-s SUM_FISH]
                  [-e] [-d] [-i INI_FILE] [-v] [-q]
                  files [files ...]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Triangle plotter')
    # parse file names:
